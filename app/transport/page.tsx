...
import { Bus, MapPin, Users, Plus, X, Clock, Pencil, Trash2, AlertTriangle } from 'lucide-react'
...
function stopsToString(stops: Stop[]): string {
  return stops.map(s => s.time ? `${s.name} ${s.time}` : s.name).join(', ')
}
function routeToForm(r: TransportRoute): FormState {
  return {
...
export default function TransportPage() {
  ...
  const openAdd = (): void => { setForm(EMPTY_FORM); setEditId(null); setModal(true) }
  const openEdit = (r: TransportRoute): void => {
  ...
  const handleUpdate = async (): Promise<void> => {
    if (!editId || !form.routeName.trim() || !form.busNumber.trim()) return
    try {
      setSaving(true)
  ...
  const confirmDelete = (r: TransportRoute): void => {
    setDeleteTarget(r)
    setDeleteModal(true)
  }
            ...
            <div key={r._id} className="card" style={{ overflow: 'hidden' }}>
              ...
              <div style={{ padding: 20 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 12, fontSize: 13, color: '#475569' }}>
                  <div>👨‍✈️ Driver: <strong>{r.driver?.name}</strong>
                    {r.driver?.phone && <span style={{ color: '#94a3b8' }}> · {r.driver.phone}</span>}
                  </div>
                  {r.conductor?.name && (
                    <div>🎫 Conductor: <strong>{r.conductor.name}</strong>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && closeModal()}>
          <div className="modal" style={{ maxWidth: 560, width: '100%' }}>
            <div className="modal-header">
              <h3>{editId ? '✏️ Edit Route' : '🚌 Add New Route'}</h3>
            ...
            <div className="modal-footer">
              ...
              <button
                ...
                disabled={saving}
              >
                {saving ? 'Saving…' : editId ? 'Update Route' : 'Add Route'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setDeleteModal(false)}>
          <div className="modal" style={{ maxWidth: 420, width: '100%' }}>
            <div className="modal-header">
              ...
              <button className="btn btn-outline btn-sm btn-icon" onClick={() => setDeleteModal(false)}><X size={16} /></button>
            </div>
            <div className="modal-body">
              ...
              <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, padding: '12px 16px' }}>
                <div style={{ fontWeight: 700, color: '#1e3a5f', fontSize: 15 }}>{deleteTarget.routeName}</div>
                <div style={{ fontSize: 13, color: '#64748b', marginTop: 2 }}>
                  Bus: {deleteTarget.busNumber} · Route #{deleteTarget.routeNumber}
                </div>
                <div style={{ fontSize: 13, color: '#64748b', marginTop: 2 }}>
                  {deleteTarget.students?.length ?? 0} students enrolled
                </div>
              </div>
            ...
            <div className="modal-footer">
              ...
              <button
                className="btn"
                style={{ background: '#dc2626', color: '#fff', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}
