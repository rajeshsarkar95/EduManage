...
const PRIMARY_TIMETABLE = (_section: string): DaySchedule => ({
    ...
    <AppLayout title="Timetable" subtitle="View weekly class schedules">
        ...
        <div style={{
          ...
          <div>
            <div style={{ color: '#fff', fontWeight: 700, fontSize: 15 }}>
              A.R. Public School — {cls} Weekly Timetable
            </div>
      ...
      <div className="card">
        ...
        <div style={{ overflowX: 'auto', padding: 20 }}>
          <table style={{ borderCollapse: 'separate', borderSpacing: '4px', minWidth: 640 }}>
            <thead>
              <tr>
                <th style={{
                ...
                <tr key={day}>
                  ...
                  {(timetable[day] || []).slice(0, periods.length).map((subj, pi) => (
                    <td key={pi} style={{
                      ...
                      fontWeight: 600, fontSize: 12, color: '#1a202c',
                      whiteSpace: 'nowrap',
                    }}>
                      {subj === '—' ? (
        ...
        <div style={{ padding: '0 20px 20px' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {Array.from(
              new Set(
                DAYS.flatMap(d => timetable[d] || []).filter(s => s !== '—')
              )
            ).map(subj => (
              <span key={subj} style={{
                background: SUBJECT_COLORS[subj] || '#f8fafc',
